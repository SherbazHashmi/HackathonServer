...
def SetErrorHandler(*args):
    r"""SetErrorHandler(char const * pszCallbackName=None) -> CPLErr"""
...
def VSIFOpenExL(*args):
    r"""VSIFOpenExL(char const * utf8_path, char const * pszMode, int bSetError) -> VSILFILE *"""
...
class Dataset(MajorObject):
    ...
    def ReadRaster1(self, *args, **kwargs):
        r"""ReadRaster1(Dataset self, int xoff, int yoff, int xsize, int ysize, int * buf_xsize=None, int * buf_ysize=None, GDALDataType * buf_type=None, int band_list=0, GIntBig * buf_pixel_space=None, GIntBig * buf_line_space=None, GIntBig * buf_band_space=None, GDALRIOResampleAlg resample_alg=GRIORA_NearestNeighbour, GDALProgressFunc callback=0, void * callback_data=None) -> CPLErr"""
        ...
        if sd is None or len(sd) == 0:
            ...
            if sd is not None and len(sd):
              ...
              if len(name_list) and len(desc_list):
                sd_list = name_list + desc_list
                sd_list[::2] = name_list 
                sd_list[1::2] = desc_list 
            return sd_list
